...
test.describe('Cart', () => {
  ...
  test('product sorting', async ({ page }) => {
    await inventoryPage.sortFromLowToHigh();

    const convertPriceToText = await inventoryPage.itemPrice.allTextContents();
    const prices = convertPriceToText.map(price => parseFloat(price.replace('$', '')));

    const sortPriceFromLowToHigh = [...prices].sort((a, b) => a - b);
    await expect(prices, 'Product prices are displayed in ascending order from low to high').toEqual(sortPriceFromLowToHigh);
  });
});